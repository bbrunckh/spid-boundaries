...
# save list of missing to check
write_xlsx(st_drop_geometry(missed_regions2),
...
# update missing boundary list in spid master excel
diff_miss <- setdiff(st_drop_geometry(missed_regions2), 
                     read_xlsx(spid_master, sheet = "SPID missing boundaries"))
diff_miss
...
dropped # small islands cropped out

# save list to check
write_xlsx(st_drop_geometry(dropped),
...
#------------------------------------------------------------------------------#
# Save subnational boundary data (not edge-matched, clipped to admin-0)
#------------------------------------------------------------------------------#

# save geopackage (NO edge-matching, clipped to admin-0)
st_write(spid_subnat,
         paste0(spid_data,"interim/",version,"/",tolower(vintage),"_subnat.gpkg"),
         append=FALSE)

# save shapefile (NO edge-matching, clipped to admin-0)
st_write(spid_subnat,
         paste0(spid_data,"interim/",version,"/",tolower(vintage),"_subnat.shp"),
         append=FALSE)